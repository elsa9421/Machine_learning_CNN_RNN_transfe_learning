...
def train_model(device, dataloaders, dataset_sizes, model, criterion, optimizer, scheduler, num_epochs=25):
    ...
    if num_epochs == 0:
        ...
        # Iterate over data.
        for inputs, labels in dataloaders['val']:     #Python iterators! dataloaders['val'] contains the input image and the corresponding label associated with it
            inputs = inputs.to(device)
            labels = labels.to(device)

            # forward
            outputs = model(inputs)
            _, preds = torch.max(outputs, 1)   #torch.max returens (values,indices so taking only the indices into consideration
             #torch.max return the max along particular dimension, here dim=1
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            for inputs, labels in dataloaders[phase]:
                ...
                # multiply it with inputs.size(0) which will be 4 because it's a mini batch of size 4
                #What is the dim of inputs? torch.Size([4, 3, 224, 224])
...
def finetune(device, dataloaders, dataset_sizes, class_names):
    ...
    #When we print the model of resnet18, we see that the last layer is a fully connected layer as shown below:

    #(fc): Linear(in_features=512, out_features=1000, bias=True)
    #Thus, we must reinitialize model.fc to be a Linear layer with 512 input features and 2 output features with:

    #in general - alternatively :- num_ftrs = model_ft.fc.in_features
    ...
    model_ft=model_ft.to(device) #run on GPU, if available
...
def main():
    ...
    # The dataset can be downloaded in https://drive.google.com/file/d/1y-iCP8T93Ti1t4SlLVoVJMOsAb9rSdaH/view?usp=sharing    
    data_dir = 'C:/Users/Elsa/Documents/HW4/hw4/hymenoptera_data'
