...
def temporal_fc_forward(x, w, b):
    ...
    an linear function to transform each of those vectors into a new vector of
    dimension M.
    Inputs:
...
def temporal_fc_backward(dout, cache):
    ...
    N,T,D=x.shape
    D,M=w.shape
    dx_inter=np.dot(np.reshape(dout,(-1,M)),w.T)
    dx=np.reshape(dx_inter,x.shape)
    dw=np.dot((np.reshape(x,(-1,D)).T),(np.reshape(dout,(-1,M))))
    #dw=np.dot(x.T,dout)
    db=np.sum(np.reshape(dout,(-1,M)),axis=0)
...
def temporal_softmax_loss(x, y, mask):
    ...
    y_predicted = np.exp(x_flat - np.max(x_flat, axis=1, keepdims=True))
    y_predicted /= np.sum(y_predicted, axis=1, keepdims=True)
    loss = -np.sum(mask_flat * np.log(y_predicted[np.arange(N * T), y_flat])) / N
    gradient = y_predicted.copy()
    gradient[np.arange(N * T), y_flat] -= 1
    gradient /= N
    gradient *= mask_flat[:, None]
    dx = gradient.reshape(N, T, V)
